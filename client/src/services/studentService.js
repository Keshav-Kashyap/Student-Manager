 ...
 import { API_BASE } from "../config/api";
  ...
  const getHeaders = (isFormData = false) => {
    ...
    console.log('🔍 Auth Debug:', { token: token ? 'EXISTS' : 'NULL', userId });
    ...
    // 🔥 FIX: Use lowercase header name to match backend
    ...
    if (token) {
      ...
      console.log('ℹ️ Using demo mode with x-user-id:', userId);
    }
  ...
  export const isAuthenticated = () => {
    try {
      ...
      console.log('🔍 Auth check:', { hasToken: !!token, hasUserId: !!userId, isAuth });
      return isAuth;
    } catch {
      return false;
    }
